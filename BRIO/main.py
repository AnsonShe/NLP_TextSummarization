...
def base_setting(args):
    ...
    args.model_type = getattr(args, "model_type", "./bart-large-cnn") # model type
    ...
    args.max_lr = getattr(args, "max_lr", 1e-2) # max learning rate (* 1e-2)
...
def evaluation(args):
    ...
    if args.do_generation: 
        # Generate for user's input
        tokenizer = tok
        count = 1
        bsz = 8
        model.generation_mode()
        while(True):    
            # Always stay in a state of waiting for user input to prevent the need to initialize the model each time, 
            # which consumes too much time.
            user_input = input("Enter text to summarize: \n")
            with open(f'./test/test.source', 'w') as source:
                source.write(user_input) 
            with open(f'./test/test.source') as source, open(os.path.join(root_dir, "test.out"), 'w') as fout:
